...
function PharmacyBilling() {
  ...
  const [patientIdSearchTimer, setPatientIdSearchTimer] = useState<ReturnType<typeof setTimeout> | null>(null);
  const [isSearchingPatient, setIsSearchingPatient] = useState(false);
  const [patientNotFound, setPatientNotFound] = useState(false);

  const filteredPatients =
    patientName.trim() === '' && patientId.trim() === '' && phone.trim() === ''
      ? []
      : patients.filter(
          (p) =>
            (patientName && p.name.toLowerCase().includes(patientName.toLowerCase())) ||
            (patientId && p.id.toLowerCase().includes(patientId.toLowerCase())) ||
            (patientId && p.mrn.toLowerCase().includes(patientId.toLowerCase())) ||
            (phone && p.phone.replace(/\D/g, '').includes(phone.replace(/\D/g, '')))
        );

  const selectPatient = (p: (typeof patients)[0]) => {
    setPatientName(p.name);
    setPatientId(p.id || p.mrn);
    const rawPhone = p.phone.replace(/\s/g, '');
    const phoneMatch = rawPhone.match(/^(\+\d{1,4})(\d{7,12})$/);
    if (phoneMatch) {
    ...
    setReferredDoctor(doc ? doc.name : 'Dr. Priya');
    setShowSuggestions(false);
    setActiveIndex(-1);
  ...
  const handlePatientIdChange = (val: string) => {
    ...
    if (patientIdSearchTimer) clearTimeout(patientIdSearchTimer);
  ...
  const handleNameChange = (val: string) => {
    if (validatePatientName(val) || val === '') {
      ...
      // Search matching patient by name
      const q = val.trim().toLowerCase();
      if (q.length >= 3) {
        const match = patients.find((p) => p.name.toLowerCase() === q);
        if (match) {
          selectPatient(match);
        }
      }
  ...
  const handlePhoneChange = (val: string) => {
    ...
    if (digitsOnly.length >= 7) {
      const match = patients.find((p) => p.phone.replace(/\D/g, '').includes(digitsOnly));
      if (match) {
        selectPatient(match);
      }
    }
  ...
  const handleEmailChange = (val: string) => {
    ...
    const q = val.trim().toLowerCase();
    if (q && q.includes('@')) {
      const match = patients.find((p) => p.email.toLowerCase() === q);
      if (match) {
        selectPatient(match);
      }
    }
  ...
  const handleSaveAndPrint = () => {
    if (!patientName) {
      toast.error('Please enter patient name.', { duration: 4000 });
      return;
    }
    if (!phone) {
      toast.error('Please enter patient phone number.', { duration: 4000 });
      return;
    }
    ...
    if (phone.length < minLength || phone.length > maxLength) {
      toast.error(`Phone number must be between ${minLength} and ${maxLength} digits.`, { duration: 4000 });
      return;
    }
    if (email && !validateEmail(email)) {
    ...
    if (billingItems.length === 0) {
      toast.error('Please add at least one medicine to generate a bill.', { duration: 4000 });
      return;
    }
          ...
          <div className="surface-elevated p-5 rounded-2xl flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <h3 className="font-display text-sm font-semibold flex items-center gap-2">
                ...
                {isSearchingPatient && <Loader2 className="animate-spin h-3.5 w-3.5 text-primary" />}
                {patientNotFound && (
                  <span className="text-[11px] font-normal text-amber-700 dark:text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-300/40">
                    Patient not found in system (New Patient)
                  </span>
                )}
              </h3>
              {patientId && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
              ...
              <div className="sm:col-span-1">
                ...
                <div className="relative mt-1">
                  <Input
                    id="patId"
                    placeholder="e.g. P10025 or p-1000"
                    value={patientId}
              ...
              <div className="relative sm:col-span-1">
                ...
                <p className="text-[11px] text-muted-foreground mt-1">Search or enter name</p>
              </div>
              ...
              <div className="sm:col-span-1">
                ...
                <div className="flex mt-1 gap-1.5">
                  <div className="h-10 px-3 bg-muted/60 border border-input rounded-md text-sm font-medium flex items-center justify-center flex-shrink-0 text-muted-foreground select-none">
                    +91
                  </div>
                  <Input
                    id="patPhone"
                    ...
                    maxLength={10}
                    className="bg-background h-10 text-sm flex-1 font-mono"
                    pattern="\d+"
                    title="Please enter exactly 10 digits"
                  />
                </div>
                <p className="text-[11px] text-muted-foreground mt-1">
                  Type phone to auto-fill
                </p>
              </div>
              ...
              <div className="sm:col-span-1">
                ...
                <Input
                  ...
                  type="email"
                />
                <p className="text-[11px] text-muted-foreground mt-1">Auto-fill on matching email</p>
              </div>
