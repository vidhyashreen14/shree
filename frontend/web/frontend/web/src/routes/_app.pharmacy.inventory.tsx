...
function PharmacyInventory() {
  ...
  const items = usePharmacyStore((s) => s.inventory);
  const history = usePharmacyStore((s) => s.stockHistory);
  ...
  const recordHistory = (entry: Omit<StockHistoryEntry, 'id' | 'at'>) => {
    adjustStock(entry.medicineId, entry.delta, entry.action, entry.by, entry.note);
  };
  ...
  const applyStockChange = () => {
    ...
    adjustStock(editing.id, n, action, 'You', note);
    toast.success(
      action === 'set'
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex flex-col text-xs text-muted-foreground whitespace-nowrap">
              <span className="font-medium text-foreground">{format(d, 'dd MMM yyyy')}</span>
              <span className="text-[11px] opacity-80">{format(d, 'hh:mm a')}</span>
            </div>
          );
        },
      },
  ...
  const undoBulkRestock = (batchId: string, snapshot: { id: string; stock: number }[]) => {
    snapshot.forEach((s) => {
      adjustStock(s.id, s.stock, 'set', 'You', 'Undid bulk restock');
    });
  ...
  const applyBulkRestock = () => {
    ...
    if (!Number.isFinite(n) || n <= 0) {
      toast.error('Enter a quantity greater than zero', { duration: 4000 });
      return;
    }
    if (selectedItems.length === 0) {
      toast.error('Select at least one medicine', { duration: 4000 });
      return;
    }
    selectedItems.forEach((m) => {
      adjustStock(m.id, n, 'add', 'You', `Bulk restock (+${n})`);
    });
  ...
  const exportCsv = () => {
    if (sorted.length === 0) {
      toast.error('Nothing to export', { duration: 4000 });
      return;
    }
    const headers = [
      'Name',
                ...
                <tr>
                  <td colSpan={columns.length} className="px-4 py-10">
                    <EmptyState
                      ...
                      description="No stock available. Save an invoice under Invoices tab or add new stock."
                    />
                  </td>
                </tr>
              ) : (
...
function ThresholdsDrawer({
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setDebouncedQuery(overrideQuery.trim().toLowerCase());
    }, 300);
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((m) => {
      const nameMatch = m.name.toLowerCase().includes(debouncedQuery);
      const batchMatch = m.batch.toLowerCase().includes(debouncedQuery);
      const catMatch = m.category.toLowerCase().includes(debouncedQuery);
      const mfgMatch = m.manufacturer.toLowerCase().includes(debouncedQuery);
      const minVal = effectiveMin(m);
      const minMatch = String(minVal).includes(debouncedQuery);
      return nameMatch || batchMatch || catMatch || mfgMatch || minMatch;
    });
  }, [items, debouncedQuery, effectiveMin]);

  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-none max-w-full w-[96vw] h-[92vh] my-auto flex flex-col p-6 overflow-hidden border border-border rounded-2xl shadow-2xl">
        <SheetHeader className="pb-2 flex-shrink-0">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <SheetTitle className="text-xl font-display font-bold">
                Low-Stock Thresholds & Reorder Levels
              </SheetTitle>

              <SheetDescription className="text-xs text-muted-foreground mt-0.5">
                Full-screen viewport threshold manager. Search by medicine name, batch, category, reorder level, or manufacturer.
              </SheetDescription>
            </div>
            
            {/* Global Search Bar specifically for threshold records */}
            <div className="relative w-full sm:w-80">
              <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
              <Input
                value={overrideQuery}
                onChange={(e) => setOverrideQuery(e.target.value)}
                placeholder="Search name, batch, category, min stock, manufacturer..."
        ...
        <Tabs defaultValue="overrides" className="mt-2 flex-1 flex flex-col min-h-0 overflow-hidden">
          <TabsList className="grid w-full sm:w-80 grid-cols-2 flex-shrink-0">
            <TabsTrigger value="overrides">Threshold Records ({filteredItems.length})</TabsTrigger>
            <TabsTrigger value="categories">Category Defaults ({categories.length})</TabsTrigger>
          </TabsList>

          <TabsContent value="overrides" className="mt-3 flex-1 flex flex-col min-h-0 overflow-hidden space-y-3">
            <div className="flex-1 overflow-y-auto pr-1 space-y-2 border border-border/80 rounded-xl p-3 bg-muted/20">
              {filteredItems.map((m) => {
                const override = thresholds.overrides[m.id];
                const eff = effectiveMin(m);
                const isLow = m.stock <= eff;
                return (
                  <div
                    key={m.id}
                    className={`flex flex-wrap items-center justify-between gap-3 rounded-xl border px-4 py-3 transition-colors ${
                      isLow
                        ? 'border-amber-300/60 bg-amber-500/5 dark:bg-amber-950/20'
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <StatusChip tone={isLow ? 'warning' : 'success'}>
                          Stock: {m.stock}
                        </StatusChip>
                      </div>
                      <p className="text-xs text-muted-foreground mt-0.5">
                        <span className="font-mono bg-muted px-1.5 py-0.5 rounded text-[11px]">Batch: {m.batch}</span> · {m.category} · Mfg: {m.manufacturer} · <span className="font-medium text-foreground">Reorder Level: {eff} units</span>
                      </p>
                    </div>

                    <div className="flex items-center gap-2 flex-shrink-0">
                      <Label htmlFor={`override-${m.id}`} className="text-xs text-muted-foreground hidden sm:inline">
                        Reorder Level:
                      </Label>
                      <Input
                        id={`override-${m.id}`}
              ...
              {filteredItems.length === 0 && (
                <div className="py-12 text-center flex flex-col items-center justify-center">
                  <EmptyState
                    ...
                    title="No matching threshold records"
                    description={
                      overrideQuery
                        ? `No items match "${overrideQuery}". Try searching by medicine name, category, batch ID, reorder level, or manufacturer.`
                        : 'No medicines in catalog.'
                    }
                  />
                  {overrideQuery && (
                    <Button variant="outline" size="sm" className="mt-3" onClick={() => setOverrideQuery('')}>
                      Clear search filter
                    </Button>
                  )}
                </div>
          ...
          <TabsContent value="categories" className="mt-3 flex-1 overflow-y-auto pr-1 space-y-3">
            {categories.length === 0 && (
              <EmptyState
                icon={SlidersHorizontal}
                title="No categories"
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {value == null && ' · using default minimums'}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Input
                  ...
                  <div className="flex flex-wrap items-center gap-3 border-t border-border/60 pt-3">
                    ...
                    >
                      <Gauge className="mr-1.5 h-3.5 w-3.5" /> Apply to {catItems.length} items
                    </Button>
                  </div>
                </div>
              );
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between border-t border-border pt-4 flex-shrink-0 gap-3">
          <p className="text-xs text-muted-foreground">
            {Object.keys(thresholds.categories).length} category rule(s) ·{' '}
            {Object.keys(thresholds.overrides).length} medicine override(s)
          </p>
          <div className="flex gap-2">
