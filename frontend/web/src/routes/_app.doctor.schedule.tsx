      ...
      <div className="surface-elevated overflow-x-auto">
        <table className="min-w-[760px] w-full text-sm">
          <thead className="bg-muted/40">
            <tr>
              <th className="w-16 px-2 py-3 text-left text-xs font-semibold uppercase text-muted-foreground">Time</th>
              {days.map((d) => (
                <th key={d.toISOString()} className="px-2 py-3 text-left text-xs font-semibold uppercase text-muted-foreground">
                  <div>{format(d, "EEE")}</div>
                  <div className={isSameDay(d, new Date()) ? "text-primary" : ""}>{format(d, "MMM d")}</div>
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {hours.map((h) => (
              <tr key={h} className="border-t border-border">
                <td className="px-2 py-3 align-top text-xs text-muted-foreground">{format(new Date().setHours(h, 0, 0, 0), "p")}</td>
                {days.map((d) => {
                  const a = slot(d, h);
                  if (!a) return <td key={d.toISOString()} className="h-16 border-l border-border p-1"><div className="h-full rounded-md border border-dashed border-transparent hover:border-border" /></td>;
