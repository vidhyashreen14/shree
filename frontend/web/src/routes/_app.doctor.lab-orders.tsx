...
function DoctorLabs() {
  ...
  const columns = useMemo<ColumnDef<LabOrder>[]>(() => [
    ...
    { header: "Ordered", accessorKey: "orderedOn", cell: ({ getValue }) => format(new Date(String(getValue())), "MMM d, yyyy") },
    {
      header: "Status",
      accessorKey: "status",
      cell: ({ getValue }) => <StatusChip tone={toneFor[getValue() as LabOrder["status"]]}>{String(getValue())}</StatusChip>,
    },
    {
      header: "",
      id: "a",
      cell: ({ row }) =>
        row.original.status === "completed" ? (
          <Button size="sm" variant="outline"><Download className="mr-1 h-3.5 w-3.5" /> Report</Button>
        ) : null,
    },
  ], []);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Button><FlaskConical className="mr-2 h-4 w-4" /> Order new test</Button>}
      />
