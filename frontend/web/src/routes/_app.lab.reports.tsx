...
const STATUS_OPTIONS = ["Select Service Status", "ordered", "sample-collected", "in-progress", "completed"];
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative group">
                ...
                <div className="absolute right-0 mt-1 hidden group-hover:block w-40 rounded-lg border border-border bg-popover py-1.5 shadow-md z-20 text-xs">
                  <button onClick={() => toast.info("Exporting all visible reports")} className="w-full text-left px-3 py-1.5 hover:bg-muted">Export All</button>
                  <button onClick={() => toast.info("Printing selected reports")} className="w-full text-left px-3 py-1.5 hover:bg-muted">Print Selected</button>
                </div>
              </div>
            </div>
          }
        ...
        {showFilterPanel && (
          <div className="surface-elevated p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
              ...
              {/* Service Status */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Service Status</label>
                <div className="relative">
                  <select
                    value={serviceStatus}
                    ...
                    className="w-full appearance-none rounded-lg border border-border bg-background px-3 py-2 text-xs text-foreground outline-none pr-8 h-9"
                  >
                    {STATUS_OPTIONS.map((o) => <option key={o} value={o}>{o === "Select Service Status" ? o : o.replace("-", " ")}</option>)}
                  </select>
                  <ChevronDown className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground" />
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1.5 sm:col-span-2 xl:col-span-2">
                <label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Lab ID / Specimen ID</label>
              ...
              {/* Mobile */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Mobile</label>
                <Input
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
              ...
              {/* B2B Dropdown */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">B2B Partner</label>
                <div className="relative">
                  <select
                    value={b2b}
              ...
              {/* Consulting Doctor */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Consulting Doctor</label>
                <div className="relative">
                  <select
            ...
            <div className="flex justify-end gap-2 border-t border-border pt-3">
              ...
              <Button size="sm" variant="outline" onClick={handleClear} className="flex items-center gap-1.5">
        ...
        {hasSearched && (
          filteredReports.length === 0 ? (
            <div className="surface-elevated p-16 text-center text-muted-foreground text-sm flex flex-col items-center gap-3">
              ...
              <Button size="sm" variant="outline" onClick={handleClear}>Reset Filters</Button>
            </div>
          ) : layoutMode === "list" ? (
            /* List/Table view */
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/30">
                    <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Lab ID</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Patient Name</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Tests</th>
                    <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground">Ordered On</th>
                  ...
                  {filteredReports.map((report) => {
                    ...
                    return (
                      <tr key={report.id} className="hover:bg-muted/20 transition-colors">
                        <td className="px-4 py-3 font-mono text-xs font-semibold text-primary">{report.id}</td>
                        <td className="px-4 py-3 font-medium text-sm">{p?.name || "Unknown Patient"}</td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex justify-center gap-1">
                            <Button size="sm" variant="ghost" className="h-8 w-8 p-0" title="Print Report">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {filteredReports.map((report) => {
                ...
                return (
                  <div key={report.id} className="surface-elevated p-4 flex flex-col justify-between gap-3 hover:shadow-md transition-shadow">
                    <div className="space-y-2">
                      <div className="flex items-center justify-between border-b border-border pb-2">
                        <code className="font-mono text-xs font-bold text-primary">{report.id}</code>
                        <StatusChip tone={toneFor[report.status] || "neutral"}>
                          {report.status.replace("-", " ")}
                        </StatusChip>
                      </div>
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-semibold">{p?.name || "Unknown Patient"}</span>
                        </div>
