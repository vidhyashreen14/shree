...
import {
  Search, UserPlus, UserCheck, ChevronRight, ChevronLeft,
  Stethoscope, Building2, IndianRupee, CheckCircle2, CreditCard,
  Banknote, Smartphone, Globe, Printer, MessageSquare, Mail, Phone, X,
} from "lucide-react";
...
function StepLookup({
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <p className="text-sm text-muted-foreground mb-6">Is this a new patient or an existing one?</p>

      {/* Toggle */}
      <div className="mb-6 grid grid-cols-2 gap-3">
        {[
          { id: "new" as const, label: "New Patient", icon: UserPlus, desc: "First time visit — needs registration" },
          { id: "existing" as const, label: "Existing Patient", icon: UserCheck, desc: "Has a UHID — search below" },
        ...
        <div className="space-y-4">
          ...
          {query.length >= 2 && (
            <div className="rounded-xl border bg-card shadow-sm divide-y">
              {results.length === 0 ? (
                <p className="py-6 text-center text-sm text-muted-foreground">No patients found for "{query}"</p>
              ) : (
...
const emptyRegForm: RegForm = {
  ...
  mobile: "", email: "", address: "", bloodGroup: "O+",
...
function StepRegistration({
  ...
  const valid = form.name.trim().length >= 2 && form.mobile.trim().length >= 7 && form.gender && form.address.trim().length >= 2 && form.emergencyName.trim().length >= 2 && form.emergencyPhone.trim().length >= 7;
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <section className="surface-elevated p-6">
        <h3 className="font-display font-semibold mb-4 flex items-center gap-2">
          <span className="grid h-7 w-7 place-items-center rounded-lg bg-primary/10 text-primary text-xs font-bold">1</span>
          Personal Details
        </h3>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label="Full Name *" className="sm:col-span-2">
            <Input id="reg-name" value={form.name} onChange={(e) => update("name", e.target.value)} placeholder="e.g. Aarav Sharma" />
          </Field>
          <Field label="Date of Birth">
            <Input id="reg-dob" type="date" value={form.dob} onChange={(e) => update("dob", e.target.value)} />
          </Field>
          <Field label="Age">
            <Input id="reg-age" type="number" min={0} max={130} value={form.age} onChange={(e) => update("age", e.target.value)} placeholder="Years" />
          </Field>
          <Field label="Gender *">
          ...
          <Field label="Mobile Number *">
            <Input id="reg-mobile" value={form.mobile} onChange={(e) => update("mobile", e.target.value)} placeholder="+91 98765 43210" />
          </Field>
          <Field label="Email (optional)">
            <Input id="reg-email" type="email" value={form.email} onChange={(e) => update("email", e.target.value)} placeholder="patient@email.com" />
          </Field>
          <Field label="Address *" className="sm:col-span-2">
            <Textarea id="reg-address" value={form.address} onChange={(e) => update("address", e.target.value)} rows={2} placeholder="Full address…" />
          </Field>
        </div>
      </section>
      ...
      <section className="surface-elevated p-6">
        <h3 className="font-display font-semibold mb-4 flex items-center gap-2">
          <span className="grid h-7 w-7 place-items-center rounded-lg bg-amber-500/10 text-amber-600 text-xs font-bold">2</span>
          Emergency Contact
        </h3>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label="Contact Person Name *">
            <Input id="reg-emname" value={form.emergencyName} onChange={(e) => update("emergencyName", e.target.value)} placeholder="Guardian / Relative name" />
          </Field>
          <Field label="Phone Number *">
            <Input id="reg-emphone" value={form.emergencyPhone} onChange={(e) => update("emergencyPhone", e.target.value)} placeholder="+91 …" />
          </Field>
      ...
      <section className="surface-elevated p-6">
        <h3 className="font-display font-semibold mb-4 flex items-center gap-2">
          <span className="grid h-7 w-7 place-items-center rounded-lg bg-emerald-500/10 text-emerald-600 text-xs font-bold">3</span>
          Basic Medical Information
        </h3>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label="Known Allergies">
            <Input id="reg-allergies" value={form.allergies} onChange={(e) => update("allergies", e.target.value)} placeholder="e.g. Penicillin, Peanuts" />
          </Field>
          <Field label="Existing Diseases / Conditions">
            <Input id="reg-diseases" value={form.diseases} onChange={(e) => update("diseases", e.target.value)} placeholder="e.g. Diabetes, Hypertension" />
          </Field>
          <Field label="Current Medications" className="sm:col-span-2">
            <Input id="reg-medications" value={form.medications} onChange={(e) => update("medications", e.target.value)} placeholder="e.g. Metformin 500mg, Amlodipine 5mg" />
          </Field>
        </div>
      </section>
      ...
      <section className="surface-elevated p-6">
        <h3 className="font-display font-semibold mb-1 flex items-center gap-2">
          <span className="grid h-7 w-7 place-items-center rounded-lg bg-blue-500/10 text-blue-600 text-xs font-bold">4</span>
          Insurance Details
          <span className="ml-1 rounded-full bg-muted px-2 py-0.5 text-[10px] font-medium text-muted-foreground">Optional</span>
        </h3>
        <p className="text-xs text-muted-foreground mb-4">Leave blank if patient doesn't have insurance.</p>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label="Insurance Provider">
            <Input id="reg-ins-provider" value={form.insuranceProvider} onChange={(e) => update("insuranceProvider", e.target.value)} placeholder="e.g. Star Health, HDFC Ergo" />
          </Field>
          <Field label="Policy Number">
            <Input id="reg-ins-policy" value={form.policyNumber} onChange={(e) => update("policyNumber", e.target.value)} placeholder="e.g. SH-100245" />
          </Field>
        </div>
      </section>
...
function StepDoctorSelection({
  ...
  const deptDoctors = doctors.filter(
    (d) => !selectedDeptId || d.department === departments.find((dep) => dep.id === selectedDeptId)?.name
  );
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          {departments.map((dept) => (
            <button
              key={dept.id}
              type="button"
              onClick={() => { setSelectedDeptId(dept.id); setSelectedDoctorId(""); }}
              className={cn(
      ...
      <section className="surface-elevated p-6">
        ...
        {!selectedDeptId ? (
          <p className="py-6 text-center text-sm text-muted-foreground">← Select a department first</p>
        ) : (
          <div className="divide-y divide-border">
              ...
              >
                ...
                <div className="text-right shrink-0">
                  ...
                  <span className={cn(
                    ...
                    doc.available ? "bg-emerald-500/10 text-emerald-600" : "bg-muted text-muted-foreground"
                  )}>
            ...
            {deptDoctors.length === 0 && (
              <p className="py-6 text-center text-sm text-muted-foreground">No doctors in this department</p>
            )}
          </div>
        )}
      </section>
      ...
      <div className="flex justify-between gap-3">
        ...
        <Button onClick={onNext} disabled={!selectedDeptId || !selectedDoctorId} id="btn-doctor-next">
...
function ConsultationBillModal({
  patientName, uhid, patientAge, patientGender,
  doctorName, department, isNewPatient,
  registrationFee, consultationFee, total, paymentMethod,
  billNo, patientPhone, patientEmail,
  onClose,
}: {
  patientName: string; uhid: string; patientAge: number; patientGender: string;
  doctorName: string; department: string; isNewPatient: boolean;
  registrationFee: number; consultationFee: number; total: number; paymentMethod: string;
  billNo: string; patientPhone: string; patientEmail: string;
  onClose: () => void;
}) {
  ...
  const handleSMS = () => {
    toast.info("SMS gateway integration required.", { description: "Connect your SMS provider API in production." });
  };
          ...
          <div ref={printRef}>
            ...
            <div className="header" style={{ display: "flex", flexDirection: "column", alignItems: "center", borderBottom: "2px solid #0d9488", paddingBottom: 16, marginBottom: 20 }}>
              {logoUrl ? (
                ...
                <div className="logo" style={{ fontSize: 22, fontWeight: 800, color: "#0d9488" }}>🏥 {name}</div>
              )}
              {logoUrl && <div style={{ fontSize: 14, fontWeight: 800, color: "#0d9488", marginTop: 2 }}>{name}</div>}
              <div className="subtitle" style={{ fontSize: 11, color: "#555", marginTop: 2 }}>Multispecialty Hospital · Compassionate Care</div>
              <div className="addr" style={{ fontSize: 11, color: "#555", marginTop: 6, textAlign: "center" }}>
                {address} · 📞 {phone}
              </div>
            </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
              <span className="badge" style={{ display: "inline-block", background: "#f0fdfa", color: "#0d9488", borderRadius: 4, padding: "2px 8px", fontSize: 11, fontWeight: 600 }}>Consultation Bill</span>
            ...
            {/* Patient info */}
            <div className="section" style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.5px", color: "#0d9488", borderBottom: "1px solid #e2e8f0", paddingBottom: 4, margin: "14px 0 8px" }}>Patient Details</div>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
              <div>
                <p style={{ fontSize: 12, marginBottom: 4 }}><strong>Patient Name:</strong> {patientName}</p>
                <p style={{ fontSize: 12, marginBottom: 4 }}><strong>Patient ID (UHID):</strong> {uhid}</p>
                <p style={{ fontSize: 12, marginBottom: 4 }}><strong>Age / Gender:</strong> {patientAge}y / {patientGender}</p>
              </div>
              <div style={{ textAlign: "right" }}>
                <p style={{ fontSize: 12, marginBottom: 4 }}><strong>Assigned Consultant:</strong> {doctorName}</p>
                <p style={{ fontSize: 12, marginBottom: 4 }}><strong>Department:</strong> {department}</p>
                <p style={{ fontSize: 12, marginBottom: 4 }}><strong>Type:</strong> {isNewPatient ? "New Patient" : "Returning Patient"}</p>
              </div>
            </div>

            {/* Fee table */}
            <div className="section" style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.5px", color: "#0d9488", borderBottom: "1px solid #e2e8f0", paddingBottom: 4, margin: "14px 0 8px" }}>Fee Details</div>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  <th style={{ background: "#f0fdfa", color: "#0d9488", fontSize: 11, fontWeight: 700, textTransform: "uppercase", padding: "8px 10px", textAlign: "left" }}>#</th>
                  <th style={{ background: "#f0fdfa", color: "#0d9488", fontSize: 11, fontWeight: 700, textTransform: "uppercase", padding: "8px 10px", textAlign: "left" }}>Description</th>
                  <th style={{ background: "#f0fdfa", color: "#0d9488", fontSize: 11, fontWeight: 700, textTransform: "uppercase", padding: "8px 10px", textAlign: "right" }}>Amount (₹)</th>
                </tr>
              </thead>
              <tbody>
                {isNewPatient && (
                  <tr>
                    <td style={{ padding: "7px 10px", borderBottom: "1px solid #f1f5f9", fontSize: 12 }}>1</td>
                    <td style={{ padding: "7px 10px", borderBottom: "1px solid #f1f5f9", fontSize: 12 }}>Hospital Registration Fee (New Patient)</td>
                    <td style={{ padding: "7px 10px", borderBottom: "1px solid #f1f5f9", fontSize: 12, textAlign: "right" }}>₹{registrationFee.toLocaleString()}</td>
                  </tr>
                )}
                <tr>
                  <td style={{ padding: "7px 10px", borderBottom: "1px solid #f1f5f9", fontSize: 12 }}>{isNewPatient ? 2 : 1}</td>
                  <td style={{ padding: "7px 10px", borderBottom: "1px solid #f1f5f9", fontSize: 12 }}>Doctor Consultation Fee — {doctorName}</td>
                  <td style={{ padding: "7px 10px", borderBottom: "1px solid #f1f5f9", fontSize: 12, textAlign: "right" }}>₹{consultationFee.toLocaleString()}</td>
                </tr>
                <tr>
                  <td colSpan={2} style={{ fontWeight: 800, fontSize: 14, borderTop: "2px solid #0d9488", padding: "10px 10px 7px" }}>Total Paid</td>
                  <td style={{ fontWeight: 800, fontSize: 14, borderTop: "2px solid #0d9488", padding: "10px 10px 7px", textAlign: "right" }}>₹{total.toLocaleString()}</td>
                </tr>
              </tbody>
            </table>
            ...
            <div style={{ marginTop: 24, textAlign: "center", fontSize: 11, color: "#888", borderTop: "1px dashed #ddd", paddingTop: 12 }}>
    ...
    <div className="mx-auto max-w-2xl space-y-5">
      {showBill && (
        <ConsultationBillModal
          ...
          onClose={() => { setShowBill(false); onComplete(); }}
        />
      )}
      ...
      <div className="surface-elevated p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{uhid} · {patientAge}y · {patientGender}</p>
          </div>
          <span className={cn("ml-auto rounded-full px-2.5 py-1 text-xs font-semibold",
        ...
        <div className="space-y-2.5">
          {isNewPatient && (
            <div className="flex items-center justify-between rounded-lg bg-muted/40 px-4 py-2.5">
              <span className="text-sm text-muted-foreground">Hospital Registration Fee <span className="text-[10px] ml-1 text-blue-500 font-semibold">(new patient)</span></span>
      ...
      <div className="surface-elevated p-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          {PAYMENT_METHODS.map(({ id, label, icon: Icon }) => (
            ...
            >
              <Icon className={cn("h-5 w-5", paymentMethod === id ? "text-primary" : "text-muted-foreground")} />
              <span className={cn("text-sm font-medium", paymentMethod === id ? "text-primary" : "")}>{label}</span>
            </button>
          ))}
        </div>
      </div>
...
function SuccessScreen({ name, uhid, onRegisterAnother }: { name: string; uhid: string; onRegisterAnother: () => void }) {
  ...
  return (
    <div className="mx-auto max-w-md text-center py-8">
      ...
      <p className="text-muted-foreground mb-1">
        <span className="font-semibold text-foreground">{name}</span> has been registered and sent to the Nurse Station for vitals.
      </p>
      <div className="my-5 inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-2">
        <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Patient ID</span>
      ...
      <div className="flex flex-col gap-3 sm:flex-row justify-center">
        <Button variant="outline" onClick={() => navigate({ to: "/frontdesk" })} id="btn-back-to-desk">
          Back to Reception
        </Button>
...
function RegisterPatient() {
  ...
  const handleNewPatient = () => { setIsNewPatient(true); setStep(1); };
  ...
  const handleRegComplete = () => {
    ...
    const newPatient: Patient = {
      ...
      insurance:
        regForm.insuranceProvider.trim()
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-4xl">
        ...
        {step === 0 && (
          <StepLookup onNew={handleNewPatient} onExisting={handleExistingPatient} />
        )}
        {step === 1 && (
          <StepRegistration
            form={regForm}
