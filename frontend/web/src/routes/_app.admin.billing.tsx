...
import {
  IndianRupee, Plus, Trash2, RotateCcw, ChevronDown, ChevronUp,
  FlaskConical, Scan, Scissors, Pill, Syringe, Dumbbell,
  HeartPulse, Shield, Building2, BedDouble, Stethoscope,
  Monitor, ClipboardList, FileText, CreditCard, Receipt, Package,
} from "lucide-react";
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">
            {category.items.filter((i) => i.enabled).length} active items · Default total ₹{total.toLocaleString()}
          </p>
        </div>
      ...
      {expanded && (
        <div className="border-t border-border px-5 pb-5 pt-4 space-y-3">
          {category.items.map((item) => (
            <div key={item.id} className={cn("flex items-center gap-3 rounded-lg border px-3 py-2.5 transition-all", !item.enabled && "opacity-50")}>
              <Switch
...
function BillingConfiguration() {
  const { registrationFee, consultationFee, setRegistrationFee, setConsultationFee, categories, resetToDefaults } = useBillingStore();
  ...
  const saveFees = () => {
    ...
    if (isNaN(r) || isNaN(c) || r < 0 || c < 0) { toast.error("Enter valid amounts"); return; }
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Button variant="outline" onClick={() => { resetToDefaults(); setRegFee("100"); setConsFee("500"); toast.success("Reset to defaults"); }}>
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          <div>
            ...
            <div className="relative">
              ...
              <Input id="admin-reg-fee" type="number" min={0} value={regFee} onChange={(e) => setRegFee(e.target.value)} className="pl-7" />
            </div>
          </div>
          <div>
            ...
            <div className="relative">
              <span className="pointer-events-none absolute inset-y-0 left-3 flex items-center text-muted-foreground text-sm">₹</span>
              <Input id="admin-cons-fee" type="number" min={0} value={consFee} onChange={(e) => setConsFee(e.target.value)} className="pl-7" />
            </div>
          </div>

          {/* Live preview */}
          <div className="sm:col-span-2 rounded-xl border bg-muted/30 p-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-3">Receipt Preview</p>
            <div className="space-y-1.5 text-sm">
              <div className="flex justify-between"><span className="text-muted-foreground">Registration Fee</span><span className="font-medium">₹{regFee || 0}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Consultation Fee</span><span className="font-medium">₹{consFee || 0}</span></div>
              <div className="border-t border-dashed my-2" />
              <div className="flex justify-between font-bold"><span>Total (new patient)</span><span className="text-primary">₹{(Number(regFee) || 0) + (Number(consFee) || 0)}</span></div>
              <div className="flex justify-between text-xs text-muted-foreground"><span>Returning patient</span><span>₹{Number(consFee) || 0}</span></div>
            </div>
          </div>

          <div className="sm:col-span-2 flex justify-end">
            <Button onClick={saveFees} id="btn-save-opd-fees">Save OPD Fees</Button>
          </div>
        </div>
      </div>
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Expand a category to edit item rates. Toggle to enable/disable items per bill.</p>
        </div>
        <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">{categories.length} bill types</span>
      </div>
