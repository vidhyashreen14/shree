...
function LabUpload() {
  ...
  return (
    <>
      <PageHeader title="Upload lab report" description="Attach a finalised report to a patient's lab order." />

      <form
        className="surface-elevated grid grid-cols-1 gap-5 p-6 sm:grid-cols-2"
        onSubmit={(e) => { e.preventDefault(); toast.success("Report uploaded"); }}
      >
        <div>
          ...
          <Select defaultValue={labOrders[0]!.id}>
            ...
            <SelectContent>
              {labOrders.map((o) => {
                ...
                return <SelectItem key={o.id} value={o.id}>{o.id} · {p?.name} · {o.tests.join(", ")}</SelectItem>;
              })}
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="sm:col-span-2">
          ...
          <Textarea className="mt-1.5" rows={5} placeholder="Cholesterol elevated. Recommend follow-up…" />
        </div>
        ...
        <label className="sm:col-span-2 flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-border bg-muted/30 p-10 text-center transition-colors hover:bg-muted/50">
          ...
          <p className="mt-2 text-sm font-semibold">{file?.name ?? "Drop PDF or click to upload"}</p>
          <p className="text-xs text-muted-foreground">PDF · max 10MB</p>
          <input type="file" accept="application/pdf" className="hidden" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />
        </label>

        <div className="flex justify-end sm:col-span-2"><Button type="submit"><UploadCloud className="mr-2 h-4 w-4" /> Upload report</Button></div>
      </form>
    </>
  );
}
