  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Button><Plus className="mr-2 h-4 w-4" /> New department</Button>}
      />
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {departments.map((d) => (
          <div key={d.id} className="surface-elevated group relative overflow-hidden p-5 transition-shadow hover:shadow-md">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 border-t border-border pt-4">
              <div>
                <p className="flex items-center gap-1 text-xs text-muted-foreground"><Stethoscope className="h-3 w-3" /> Doctors</p>
                <p className="font-display text-xl font-bold">{d.doctorCount}</p>
              </div>
              <div>
                <p className="flex items-center gap-1 text-xs text-muted-foreground"><Users className="h-3 w-3" /> Today</p>
