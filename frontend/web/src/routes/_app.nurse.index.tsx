  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Link to="/nurse/vitals" search={{}}><Button><HeartPulse className="mr-2 h-4 w-4" /> Record vitals</Button></Link>}
      />

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Awaiting vitals (reception)" value={freshQueue.length} icon={Hourglass} tone="warning" />
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="surface-elevated p-5">
              ...
              <div key={entry.id} className="py-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">{entry.uhid} · {entry.age}y · {entry.gender}</p>
                    <p className="text-xs text-muted-foreground truncate">{entry.doctorName} · {entry.department}</p>
                    <div className="mt-1 flex items-center gap-1.5 text-[10px] text-muted-foreground">
                      ...
                      <span className={cn(
                        ...
                        entry.isNewPatient ? "bg-blue-500/10 text-blue-600" : "bg-muted text-muted-foreground"
                      )}>
          ...
          {freshQueue.length > 0 && (
            <Link to="/nurse/queue" className="mt-2 block text-center text-xs font-semibold text-primary hover:underline">
              View full queue
            </Link>
          )}
        </div>
          ...
          {awaiting.slice(0, 6).map((a) => {
            ...
            return (
              <div key={a.id} className="flex items-center gap-3 py-3">
                ...
                <Link to="/nurse/vitals" search={{ patientId: p.id }}><Button size="sm" variant="outline">Take</Button></Link>
              </div>
            );
          })}
