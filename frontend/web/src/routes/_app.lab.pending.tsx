...
const toneFor: Record<LabOrder["status"], Parameters<typeof StatusChip>[0]["tone"]> = {
  ordered: "info", "sample-collected": "primary", "in-progress": "warning", completed: "success",
};
...
function LabPending() {
  ...
  const filtered = useMemo(
    () => (tab === "all" ? labOrders.filter((l) => l.status !== "completed") : labOrders.filter((l) => l.status === tab)),
    [tab],
  );

  const columns = useMemo<ColumnDef<LabOrder>[]>(() => [
    { header: "Order", accessorKey: "id", cell: ({ getValue }) => <code className="font-mono text-xs">{String(getValue())}</code> },
    ...
    { header: "Ordered", accessorKey: "orderedOn", cell: ({ getValue }) => format(new Date(String(getValue())), "MMM d, p") },
    { header: "Status", accessorKey: "status", cell: ({ getValue }) => <StatusChip tone={toneFor[getValue() as LabOrder["status"]]}>{String(getValue())}</StatusChip> },
    {
      header: "",
      id: "a",
      cell: () => (
        <Button size="sm" variant="outline" onClick={() => toast.success("Marked sample collected")}>
