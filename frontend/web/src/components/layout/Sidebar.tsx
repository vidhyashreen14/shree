        ...
        <div className="flex h-full w-full flex-col overflow-hidden rounded-[32px] bg-sidebar text-sidebar-foreground shadow-2xl border border-sidebar-border">
          ...
          <div className="flex items-center justify-between px-4 pt-4 pb-2">
            <div className="flex items-center gap-2 max-w-[200px]">
              ...
              {user.role === 'superadmin' ? (
                <Link to="/" className="flex items-center gap-2.5 min-w-0" title="MediCore Platform">
          ...
          <div className="px-4 pt-4">
            ...
            >
              ...
              <kbd className="rounded-full border border-sidebar-border bg-sidebar px-1.5 py-0.5 font-mono text-[9px] text-muted-foreground shrink-0">
                ⌘K
              </kbd>
            </button>
          </div>
          ...
          <nav className="flex-1 space-y-6 overflow-y-auto px-4 py-4 scrollbar-none">
            ...
            <div className="space-y-2 pt-2 border-t border-sidebar-border">
              ...
              <ul className="space-y-1.5">
                {SHARED_NAV.filter((item) => item.to !== '/profile' || user.role === 'doctor').map(
                  (item) => {
                    const Icon = item.icon;
                    const active = isActive(item.to);
                    return (
